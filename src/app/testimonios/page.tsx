          ...
          <div className={(`max-w-7xl mx-auto p-4 sm:p-6 lg:p-8`)}>
            <div className={(`container mx-auto px-6 p-6 bg-white`)}>
              <div className={(`mb-16 text-center`)}>
                <h4 className={(`text-base text-[#FF5A5A] font-semibold tracking-wide uppercase`)}>Testimonios</h4>
                ...
                {testimonios.map((testimonio, index) => (
                  <div key={index} className={(`w-full lg:w-1/2 p-8`)}>
                    <div className={(`flex items-center mb-6`)}>
                      ...
                      <div className={(`ml-4 text-xl font-semibold`)}>{testimonio.name}</div>
                    </div>
                    <p className={(`text-gray-500 mb-2`)}>{testimonio.location}</p>
                    <p className={(`text-gray-500 mb-4`)}>{testimonio.date}</p>
                    <img
                      src={testimonio.image}
                      alt={`${testimonio.name}`}
                      className={(`w-full h-64 object-cover rounded-lg mb-4`)}
                    />
                    <p className={(`leading-loose text-gray-500 mb-8 whitespace-pre-line`)}>{testimonio.story}</p>
                    <p className={(`text-gray-500 font-semibold`)}>{testimonio.role}</p>
                  </div>
                ))}
              </div>
            </div>
